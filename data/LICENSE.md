...
The root `LICENSE` is MIT and covers the software, not these files. The same applies to
`demo/data/` and `analysis/figures/data/`.
...
Attribution, citations and the exact snapshot identity:
[`../THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md). Cite the sources and state that the data
was modified when you reuse anything here.
