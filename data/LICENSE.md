...
| `mhc_mapping/mhc_sources.csv` | IPD-IMGT/HLA release 3.59.0, via its GitHub distribution [`ANHIG/IMGTHLA`](https://github.com/ANHIG/IMGTHLA) | **No IPD sequence is redistributed.** This file carries the four-field allele each row was taken from and our peptide-binding window coordinates. `pipeline/preprocess/build_mhc_tables.py` fetches the alignment and rebuilds the sequence tables locally; the rebuilt files are `.gitignore`d. IPD-IMGT/HLA is CC BY-NoDerivs and its notice asks for three citations, which are in `THIRD_PARTY_NOTICES.md`. The 20 IPD-MHC rows (BoLA, SLA, Mamu) are omitted entirely. |
