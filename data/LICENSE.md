...
| `mhc_mapping/*.csv`, rows for HLA (116 of 154) | IPD-IMGT/HLA release 3.59.0, via its GitHub distribution [`ANHIG/IMGTHLA`](https://github.com/ANHIG/IMGTHLA) | **CC BY-NoDerivs**, per that repository's `LICENCE.md`. What is here is the full-length gap-free chain under two-field names, with the peptide-binding window carried as coordinates (`start_idx`/`end_idx`, or `sequence|start|end`); `HLA2_IMGT_MSA_idx.csv` also carries the cut window in `Sliced_Seq`, and `demo/data/mhc_mapping_demo.csv` is cut to it. The alignment itself is not redistributed — `pipeline/preprocess/fetch_mhc_alignment.py` downloads it. The notice asks for three citations; they are in `THIRD_PARTY_NOTICES.md`. |
