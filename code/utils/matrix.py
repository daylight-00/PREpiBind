...
aa_to_idx = {aa: i for i, aa in enumerate("LFIMVWCYHATGPRQSNEDK")}

def get_calpha_matrix(hla_seq, epi_seq, matrix_size=(9, 369)):
    encoded_matrix = np.zeros(matrix_size)
    for i, epi_aa in enumerate(epi_seq):
        for j, hla_aa in enumerate(hla_seq):
            if epi_aa in {"*", "X"} or hla_aa in {"*", "X"}: encoded_matrix[i, j] = 0
            elif epi_aa == "U": encoded_matrix[i, j] = impact_matrix[aa_to_idx["C"], aa_to_idx[hla_aa]]
            elif epi_aa == "J": encoded_matrix[i, j] = max(impact_matrix[aa_to_idx["L"], aa_to_idx[hla_aa]], impact_matrix[aa_to_idx["I"], aa_to_idx[hla_aa]])
            elif epi_aa == "Z": encoded_matrix[i, j] = max(impact_matrix[aa_to_idx["Q"], aa_to_idx[hla_aa]], impact_matrix[aa_to_idx["E"], aa_to_idx[hla_aa]])
            elif epi_aa == "B": encoded_matrix[i, j] = max(impact_matrix[aa_to_idx["D"], aa_to_idx[hla_aa]], impact_matrix[aa_to_idx["N"], aa_to_idx[hla_aa]])
            else: encoded_matrix[i, j] = impact_matrix[aa_to_idx[epi_aa], aa_to_idx[hla_aa]]
